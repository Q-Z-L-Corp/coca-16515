...
const ChatTutor: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: "Hello! I'm your vocabulary tutor. Try using a word like 'ubiquitous' or 'serendipity' in a sentence, and I'll give you feedback." }
  ]);
  ...
  return (
    <div className="max-w-3xl mx-auto h-[calc(100vh-140px)] flex flex-col bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-2xl shadow-slate-200/50">
      ...
      <div className="bg-white p-5 border-b border-slate-100 flex items-center space-x-4">
        ...
        <div>
          <h3 className="font-extrabold text-slate-800 text-lg">Professor Gemini</h3>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/50">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            >
              <div className={`flex items-center gap-2 mb-2 text-xs font-bold uppercase tracking-wider ${msg.role === 'user' ? 'text-indigo-200' : 'text-slate-400'}`}>
