...
import { BookOpen, Gamepad2, LayoutDashboard, MessageCircle, Library } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col md:flex-row font-sans">
      ...
      <aside className="hidden md:flex flex-col w-72 bg-white border-r border-slate-200 p-6 sticky top-0 h-screen shadow-sm z-10">
        <div className="mb-10 px-2">
            ...
            <p className="text-xs text-slate-400 font-medium tracking-wide uppercase mt-1">Vocabulary Master</p>
        </div>
        ...
        <nav className="space-y-3">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon size={22} className={isActive ? 'text-white' : 'text-slate-400 group-hover:text-indigo-600 transition-colors'} />
                <span className={`font-bold text-base ${isActive ? '' : 'font-semibold'}`}>{item.label}</span>
              </button>
            );
          })}
        </nav>
        ...
        <div className="mt-auto pt-6 border-t border-slate-100">
           <div className="bg-gradient-to-br from-indigo-50 to-violet-50 p-4 rounded-2xl border border-indigo-100">
               <p className="text-xs font-semibold text-indigo-900 mb-1">Powered by Gemini 3</p>
               <p className="text-[10px] text-indigo-600/70">Next-gen AI tutoring enabled</p>
           </div>
        </div>
      </aside>
