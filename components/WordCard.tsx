...
const WordCard: React.FC<WordCardProps> = ({ item, className = "", autoPlayAudio = false }) => {
          ...
          <div className="z-10 flex flex-col items-center w-full relative">
              ...
              {wordData?.phonetic ? (
                  <div className="inline-block bg-slate-50 px-4 py-1.5 rounded-full border border-slate-200">
                      <p className="text-slate-500 font-mono text-base md:text-lg font-medium">{wordData.phonetic}</p>
                  </div>
              ) : (
        ...
        >
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full space-y-3">
              ...
              <p className="text-slate-500 text-sm font-semibold">Loading definitions...</p>
            </div>
          ) : wordData ? (
            <div className="flex flex-col h-full">
              <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-4 shrink-0">
                <h3 className="text-2xl font-black text-slate-800 capitalize truncate pr-2">{wordData.word}</h3>
                <button 
                  onClick={handleSpeak} 
                  ...
                  <div>
                     ...
                     <ul className="space-y-3">
                       {wordData.examples.slice(0, 2).map((ex, i) => (
                         <li key={i} className="text-slate-600 text-xs md:text-sm bg-indigo-50/50 p-3 rounded-xl border-l-4 border-indigo-400 italic leading-relaxed">
                           "{ex}"
                         </li>
                       ))}
                     </ul>
