...
import { Trophy, Flame, Brain, Target, ArrowRight, ChevronDown } from 'lucide-react';

interface DashboardProps {
  stats: UserStats;
}

const StatCard: React.FC<{ title: string; value: string | number; icon: React.ElementType; colorClass: string; bgClass: string }> = ({ title, value, icon: Icon, colorClass, bgClass }) => (
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
        <div>
            <h2 className="text-4xl font-extrabold text-slate-800 mb-2">Hello, Scholar 👋</h2>
            <p className="text-slate-500 font-medium">Ready to expand your vocabulary today?</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-100 rounded-3xl p-8 shadow-xl shadow-slate-200/40">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-2xl font-bold text-slate-800">Learning Activity</h3>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.history}>
                ...
                <Tooltip 
                  ...
                  contentStyle={{ backgroundColor: '#1e293b', borderRadius: '12px', border: 'none', color: '#f8fafc', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}
                ...
                <Bar dataKey="xp" radius={[8, 8, 8, 8]} barSize={40}>
                  {stats.history.map((entry, index) => (
                    <Cell 
                        key={`cell-${index}`} 
                        fill={index === stats.history.length - 1 ? '#4f46e5' : '#e2e8f0'} 
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-3xl p-8 shadow-xl shadow-indigo-200 flex flex-col justify-between text-white relative overflow-hidden">
            ...
            <div>
              <div className="flex items-center space-x-2 mb-4 opacity-90">
                 <Target size={20} />
                 <span className="font-bold text-sm uppercase tracking-wider">Mastery Goal</span>
              </div>
              <h3 className="text-3xl font-black mb-2">0.8%</h3>
              <p className="text-indigo-100 font-medium leading-relaxed">You have mastered <span className="font-bold text-white">142</span> out of 16,515 words.</p>
            </div>
            ...
            <div className="mt-8">
               ...
               <button className="w-full py-4 bg-white text-indigo-700 rounded-xl font-bold hover:bg-indigo-50 transition-colors flex items-center justify-center group shadow-lg">
                 Continue Learning <ArrowRight size={18} className="ml-2 group-hover:translate-x-1 transition-transform" />
               </button>
            </div>
        </div>
      </div>
