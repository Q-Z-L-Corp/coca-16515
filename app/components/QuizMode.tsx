...
const AVAILABLE_DATA = VALID_COCA_DATA.filter((item) =>
	isWordAvailable(item.html),
);
...
const QuizMode: React.FC<QuizModeProps> = ({ onCorrectAnswer }) => {
	...
	const getWorkingDataPool = () => {
		return AVAILABLE_DATA.length > 0
			? AVAILABLE_DATA
			: VALID_COCA_DATA.slice(0, 10);
	};
	...
	const loadNewQuestion = async () => {
		...
		try {
			...
			const options = [...distractors, targetItem.word].sort(
				() => Math.random() - 0.5,
			);
		...
		return (
			<div className="text-center p-8">
				...
				<button
					...
					className="mt-4 px-6 py-2 bg-indigo-600 rounded-xl text-white font-bold shadow-lg shadow-indigo-200"
				>
					Retry
				</button>
			</div>
			...
			<div className="bg-white border border-slate-100 rounded-3xl p-6 md:p-10 shadow-xl shadow-slate-200/50">
					...
					{question.options.map((option, idx) => {
						...
						if (selectedAnswer) {
							if (isSelected && isCorrect) {
								...
								btnClass =
									"bg-rose-50 border-rose-500 text-rose-700 ring-2 ring-rose-100";
								icon = <XCircle className="text-rose-500" size={20} />;
							} else if (!isSelected && isTarget) {
								btnClass =
									"bg-emerald-50 border-emerald-500 text-emerald-700 opacity-60"; // Show correct answer
							} else {
				...
				{selectedAnswer && (
					<div className="mt-10 pt-6 border-t border-slate-100 animate-fade-in">
						<h4 className="font-bold text-slate-400 mb-3 text-xs uppercase tracking-wider">
							Example Usage
						</h4>
