...
const LETTER_STYLES = [
	...
	{
		bg: "bg-fuchsia-100",
		hover: "group-hover:bg-fuchsia-200",
         ...
         */}
         ...
         `}
             ...
             `}
					>
						<div
							className={`flex items-center text-indigo-500 mb-3 ml-1 transition-all ${!isBrowsing ? "mb-0 mr-3" : ""}`}
						>
							<Search size={!isBrowsing ? 20 : 28} strokeWidth={2.5} />
							{isBrowsing && (
								<span className="ml-2 font-black uppercase tracking-wider text-sm text-slate-400">
									Search
								</span>
							)}
						</div>
         ...
         */}
				{isBrowsing &&
                 ...
                 `}
							>
								<span
									className={`text-4xl md:text-5xl font-black transition-all drop-shadow-sm ${style.text}`}
								>
									{char}
								</span>
							</button>
			...
			{selectedWord && (
				<div
					className="fixed inset-0 z-[60] flex items-center justify-center px-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in"
